...
<template>
  <div class="flex flex-1 flex-col gap-4 p-4 overflow-y-auto">
    <h1 class="text-xl font-bold">Custom Command</h1>
    <p class="text-xs text-base-content/60">
      Enter raw FFmpeg parameters directly. The input file and output path are handled automatically.
    </p>
    <div class="alert alert-warning text-xs py-2">
      ...
      <span>Custom mode has the highest priority and completely bypasses Config and A/V Mix settings. Only the input file and output path are handled automatically.</span>
    </div>
    ...
    <div class="card bg-base-200 shadow-sm">
      <div class="card-body p-4">
        <h2 class="card-title text-sm font-semibold mb-3">FFmpeg Parameters</h2>
        <textarea
        ...
        <p class="text-xs text-base-content/50 mt-2">
          Do not include -i input or output path. Only the parameters between them.
        </p>
      </div>
    </div>
